...
def week02_tools() -> list[Any]:
    ...
    return week01_tools() # 1주차 tool 목록 그대로 반환
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(), f"""오늘은 {current_app_date_iso()}이다.
...
def build_week02_agent() -> object:
    ...
    if _WEEK02_AGENT is None:
        _WEEK02_AGENT = create_agent(
            model=chat_model(), # 어떤 LLM 모델
            tools=week02_tools(), # 어떤 tool 
            response_format=StructuredRequestBatch, # 어떤 형식으로 출력
            system_prompt=week02_system_prompt(), # 어떤 규칙으로 판단
        )
