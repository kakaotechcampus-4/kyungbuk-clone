...
class StructuredRequest(BaseModel):
    ...
    members: list[str]= Field(default_factory=list, description="멤버 목록")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="정형화된 사용자 요청 목록, 요청이 하나뿐이어도 list 형태를 유지")
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError("잘못된 LLM 응답입니다.")
...
def week02_system_prompt() -> str:
    ...
    week2_prom = join_system_prompt(week02_prompt_parts())
