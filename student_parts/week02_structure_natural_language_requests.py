...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description="요청 종류입니다. personal_schedule/group_schedule/todo/reminder 중 하나이며, 판단이 서지 않으면 unknown으로 둡니다.",
    )
    title: str | None = Field(
        default=None,
        description="일정이나 할 일의 제목입니다. 원문에서 드러나지 않으면 None으로 둡니다.",
    )
    date: str | None = Field(
        default=None,
        description="YYYY-MM-DD 형식의 날짜입니다. '내일'처럼 상대 표현은 base_date 기준으로 계산하고, 확실하지 않으면 None으로 둡니다.",
    )
    start_time: str | None = Field(
        default=None,
        description="HH:MM 형식의 시작 시각입니다. 확실할 때만 채우고 그 외에는 None으로 둡니다.",
    )
    end_time: str | None = Field(
        default=None,
        description="HH:MM 형식의 종료 시각입니다. 원문에 없으면 None으로 둡니다.",
    )
    members: list[str] = Field(
        default_factory=list,
        description="참석자나 관련 멤버 이름 목록입니다. 알 수 없으면 빈 목록으로 둡니다.",
    )
    priority: str | None = Field(
        default=None,
        description="할 일의 우선순위입니다. 원문에서 판단할 수 있을 때만 채웁니다.",
    )
    reason: str | None = Field(
        default=None,
        description="이렇게 구조화한 근거나 판단 이유입니다. 필요 없으면 None으로 둡니다.",
    )
    original_text: str = Field(
        default="",
        description="구조화하기 전 사용자의 원문을 그대로 보존하는 필드입니다.",
    )
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        default_factory=list,
        description="구조화한 요청 목록입니다. 요청이 하나뿐이어도 StructuredRequest 하나를 담은 목록으로 유지합니다.",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="'내일', '다음 주 화요일' 같은 상대 날짜를 해석하는 기준일입니다. YYYY-MM-DD 형식의 현재 앱 날짜를 사용합니다.",
    )


def _coerce_structured_request(value: Any) -> StructuredRequest:
    """structured output 결과를 StructuredRequest 하나로 정규화합니다."""
...
def extract_structured_request(text: str) -> StructuredRequest:
    """agent loop 없이 자연어/JSON 문자열을 StructuredRequest 하나로 구조화합니다.

    Week 3 이상에서 저장/조율 직전 입력을 구조화할 때 재사용하는 bridge 함수입니다.
    """
...
def week02_system_prompt() -> str:
    ...
    final_answer_rule = (
        ...
        "요청이 하나뿐이어도 requests 목록에 StructuredRequest 하나를 담고, base_date에는 상대 날짜 해석 기준일을 넣는다. "
        "개인 일정 생성 요청이라면 personal_create_schedule tool 결과 JSON의 created_schedule을 읽어 각 필드를 채운다."
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"너는 사용자의 한국어 자연어 요청을 일정 앱이 읽을 수 있는 구조화 데이터로 바꾸는 2주차 요청 구조화 agent다. "
