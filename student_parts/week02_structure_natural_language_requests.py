...
class StructuredRequestBatch(BaseModel):
    ...
    base_date: str = Field(
        ...
        description="'내일'이나 '다음 주' 같은 상대 날짜를 해석할 때 기준이 되는 오늘 날짜(YYYY-MM-DD).",
    )
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    if isinstance(value, StructuredRequest):
        return value
    # dict이면 Pydantic 검증을 거쳐 StructuredRequest로 바꾼다.
    if isinstance(value, dict):
        return StructuredRequest.model_validate(value)
    # 예상한 형태가 아니면 잘못된 LLM 응답을 조용히 통과시키지 않고 오류를 낸다.
    raise RuntimeError(f"StructuredRequest로 변환할 수 없는 structured output입니다: {type(value)!r}")


def extract_structured_request(text: str) -> StructuredRequest:
    """Week 3 이상에서 agent를 새로 띄우지 않고 자연어를 StructuredRequest로 바꿉니다."""

    # agent loop 없이 structured output만 쓰는 가벼운 bridge다.
...
def extract_schedule_request(query: str) -> str:
    ...
    # 자연어 또는 Week 1 tool JSON payload를 StructuredRequest 하나로 구조화한다.
    structured_request = extract_structured_request(query)
    payload = {
        "ok": True,
        "tool_name": "extract_schedule_request",
        "base_date": current_app_date_iso(),
        # 이후 저장 tool이 그대로 받을 수 있도록 model_dump() 결과를 담는다.
...
def week02_tools() -> list[Any]:
    ...
    # Week 1 개인 일정 tool 목록을 그대로 노출한다.
    return week01_tools()
...
def week02_system_prompt() -> str:
    ...
    batch_rule = (
        ...
        "한 문장에 일정·할 일·알림 의도가 여러 개 섞여 있으면 여러 StructuredRequest로 나눈다. "
        "personal_create_schedule tool을 호출했다면 그 결과 JSON의 created_schedule을 읽어 "
        "title/date/start_time/end_time/members 필드를 채운다."
    )
    return join_system_prompt([*week02_prompt_parts(), batch_rule])
...
def week02_prompt_parts() -> list[str]:
    ...
    week02_role = (
        ...
        "Week 2에서는 SQLite 저장, RAG, 외부 멤버 일정 조율은 하지 않는다."
    )
    return [
        *week01_prompt_parts(),
        week02_role,
...
def build_week02_agent() -> object:
    ...
    if _WEEK02_AGENT is None:
        _WEEK02_AGENT = create_agent(
            model=chat_model(),
            tools=week02_tools(),
            # ToolStrategy로 감싸 tool-call 기반 구조화를 쓴다. 맨 스키마를 넘기면 프록시 모델이
            # native 방식에서 JSON 뒤에 여분 텍스트를 붙여 파싱이 깨지는 경우가 있어 이를 피한다.
