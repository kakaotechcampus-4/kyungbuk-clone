...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="요청의 성격")
    # TODO: title/date/start_time/end_time 필드를 str | None 타입으로 선언하고 기본값은 None으로 두세요.
    title: str | None = Field(default=None, description="일정이나 할 일의 핵심 제목")
    ...
    members: list[str] = Field(default_factory=list, description="참석자 인원 목록")
    # TODO: priority/reason 필드를 str | None 타입으로 선언하고 기본값은 None으로 두세요.
    priority: str | None = Field(default=None, description="우선순위({상, 중, 하} 형식)")
    reason: str | None = Field(default=None, description="각 필드 변환의 판단 근거")
    # TODO: original_text 필드를 str 타입으로 선언하고 기본값은 ""로 두세요.
    original_text: str = Field(default="", description="사용자의 요청 텍스트 원문")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="요청들의 목록")
    # TODO: base_date 필드를 str 타입으로 선언하고 default_factory=current_app_date_iso를 사용하세요.
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜(날짜를 해석하는 데 기준이 되는 오늘 날짜)")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt([
        *week02_prompt_parts(),
        "최종 답변은 StructuredRequestBatch 포맷으로 구성.",
        "StructuredRequestBatch에 요청이 단 하나만 존재하더라도 'requests' 필드는 빈 리스트가 아니라 단일 StructuredRequest를 포함한 리스트 형식으로 저장.",
        "personal_create_schedule tool의 결과 JSON을 받았을 경우, JSON 내 'created_schedule' 필드에 담긴 payload를 파싱하여 StructuredRequestBatch의 관련 필드들을 채워야 함.",
    ])
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        "너의 역할은 자연어 요청 혹은 tool 실행 결과 JSON을 파싱하여, 일정 관리에 필요한 구조화된 정보(StructuredRequestBatch)를 생성해야 돼.",
        ...
        "이번 단계에서는 SQLite 저장, RAG, 외부 멤버 일정 조율 등은 수행하지 않아.",
    ]
