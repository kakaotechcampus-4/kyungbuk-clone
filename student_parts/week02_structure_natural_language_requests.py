...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description="요청의 종류 (personal_schedule: 개인 일정, group_schedule: 그룹 일정, todo: 할 일, reminder: 알림, unknown: 분류 불가)"
    )
    title: str | None = Field(
        default=None,
        description="일정 또는 할 일의 제목. 확실하지 않으면 None으로 둡니다."
    )
    date: str | None = Field(
        default=None,
        description="일정 날짜 (YYYY-MM-DD 형식). 확실할 때만 채우고 모르면 None으로 둡니다."
    )
    start_time: str | None = Field(
        default=None,
        description="일정 시작 시간 (HH:MM 형식). 확실할 때만 채우고 모르면 None으로 둡니다."
    )
    end_time: str | None = Field(
        default=None,
        description="일정 종료 시간 (HH:MM 형식). 확실할 때만 채우고 모르면 None으로 둡니다."
    )
    members: list[str] = Field(
        default_factory=list,
        description="참석자 또는 관련 멤버 목록. 멤버가 없거나 모르면 빈 리스트로 둡니다."
    )
    priority: str | None = Field(
        default=None,
        description="할 일의 우선순위 (높음/중간/낮음 등). 할 일이 아니거나 모르면 None으로 둡니다."
    )
    reason: str | None = Field(
        default=None,
        description="요청을 이렇게 구조화한 판단 근거. 모르면 None으로 둡니다."
    )
    original_text: str = Field(
        default="",
        description="원본 사용자 입력 텍스트를 보존하는 필드"
    )
...
class StructuredRequestBatch(BaseModel):
    ...
    base_date: str = Field(
        ...
        description="상대 날짜(내일, 다음 주 등) 해석의 기준이 되는 현재 날짜 (YYYY-MM-DD 형식)"
    )
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError(f"예상한 형태가 아닙니다: {type(value)}")
...
def week02_system_prompt() -> str:
    ...
    additional_rules = [
        "## 최종 답변 규칙\n"
        "- 최종 답변은 반드시 StructuredRequestBatch 형식으로 반환합니다.\n"
        "- 요청이 하나뿐이어도 requests 목록에 StructuredRequest 하나를 담습니다.\n"
        "- 여러 의도가 섞여 있으면 각각을 별도의 StructuredRequest로 분리합니다.\n"
        "\n"
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        f"- 사용자의 자연어 요청을 StructuredRequest 형태로 구조화합니다.\n"
        f"- kind, title, date, start_time, end_time, members, priority, reason, original_text 필드를 채웁니다.\n"
        f"- 날짜는 YYYY-MM-DD, 시간은 HH:MM 형식으로 표현합니다.\n"
        f"- 확실하지 않은 정보는 None 또는 빈 리스트로 둡니다.\n"
        f"\n"
        f"## Week 1 tool JSON 처리\n"
        f"- Week 1 도구(personal_create_schedule 등)가 반환한 created_schedule JSON을 받은 경우,\n"
        f"  다시 tool을 호출하지 않고 해당 JSON payload를 읽어서 StructuredRequest로 변환합니다.\n"
        f"\n"
        f"## Week 2 범위\n"
        f"- Week 2에서는 SQLite 저장, RAG, 외부 멤버 일정 조율을 수행하지 않습니다.\n"
        f"- 단지 자연어를 구조화된 형식으로 변환하는 것이 목표입니다.\n"
    ]
