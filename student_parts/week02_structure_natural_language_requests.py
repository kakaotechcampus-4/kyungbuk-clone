...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description = "요청 종류, personal_schedule(개인 일정), group_schedule(여러명이 참여하는 일정), todo(할 일), reminder(단순 알림), unknown(분류 불가) 중 하나.")
    ...
    original_text: str = Field(default = "", description = "사용자 원문")
...
class StructuredRequestBatch(BaseModel):
    ...
    base_date: str = Field(default_factory=current_app_date_iso, description = "상대 날짜(내일, 다음 주 등)을 해석할 때 기준이 되는 오늘 날짜(YYYY-MM-DD).")
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        (
            f"당신은 요청 구조화 agent입니다. 오늘은 {current_app_date_iso()}이며, "
            "이 날짜를 상대 날짜(예: '내일', '다음 주') 해석 기준일로 사용합니다."
        ),
        (
