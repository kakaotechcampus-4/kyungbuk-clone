...
class StructuredRequest(BaseModel):
    ...
    date: str | None = Field(
        default=None, description="YYYY-MM-DD 형식의 날짜. 상대 날짜는 base_date 기준으로 계산. 확실하지 않으면 None."
    )
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        default_factory=list, description="입력 문장에서 추출한 StructuredRequest 목록. 요청이 하나여도 리스트로 유지."
    )
    base_date: str = Field(
        default_factory=current_app_date_iso, description="상대 날짜(오늘/내일/다음 주 등)를 해석하는 기준일. YYYY-MM-DD."
    )
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError(f"예상하지 못한 structured output 형식입니다: {type(value)!r}")
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        (
            f"당신은 이제 Week 2 요청 구조화 agent입니다. 오늘 날짜는 {current_app_date_iso()}입니다. "
            "사용자의 자연어 요청이나 personal_create_schedule 같은 tool의 JSON 결과를 읽어 "
            ...
            "그 payload의 title/date/start_time/end_time/attendees 값을 읽어 StructuredRequest 필드를 채우세요. "
