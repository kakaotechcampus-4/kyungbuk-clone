...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="이 요청이 어떤 종류인지: personal_schedule, group_schedule, todo, reminder, unknown 중 하나")
    title: str | None = Field(default=None, description="일정/할 일 제목")
    date: str | None = Field(default=None, description="일정/할 일 날짜 (YYYY-MM-DD), 확실할 때만 채움")
    start_time: str | None = Field(default=None, description="시작 시각 (HH:MM), 확실할 때만 채움")
    end_time: str | None = Field(default=None, description="종료 시각 (HH:MM), 확실할 때만 채움")
    members: list[str] = Field(default_factory=list, description="참석자/관련 멤버 목록, 모르면 빈 list")
    priority: str | None = Field(default=None, description="할 일 우선순위, 확실할 때만 채움")
    reason: str | None = Field(default=None, description="구조화 판단 근거, 확실할 때만 채움")
    original_text: str = Field(default="", description="사용자 입력 원문 텍스트")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="자연어에서 추출된 구조화 요청 목록")
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜 해석 기준일 (오늘 날짜, YYYY-MM-DD)")
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        f"너는 자연어 요청과 Week 1 tool 결과를 StructuredRequestBatch로 구조화하는 역할도 겸한다. 오늘 날짜는 {current_app_date_iso()}이다.",
        "사용자의 자연어 요청을 kind/title/date/start_time/end_time/members 등 StructuredRequest 필드로 정리해 최종 답변으로 낸다.",
        "Week 1 tool을 호출해 created_schedule 같은 JSON 결과를 이미 받았다면, tool을 다시 호출하지 말고 그 결과를 읽어 structured_response로 정리한다.",
        "이 단계에서는 SQLite 저장, RAG 검색, 외부 멤버 일정 조율은 하지 않는다.",
    ]
