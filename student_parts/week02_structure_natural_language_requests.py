...
class StructuredRequest(BaseModel):
    ...
    title: str | None = Field(default=None, description="일정 또는 할 일의 제목입니다. 확실하지 않으면 None으로 둡니다.")
    date: str | None = Field(default=None, description="YYYY-MM-DD 형식의 날짜입니다. 확실하지 않으면 None으로 둡니다.")
    start_time: str | None = Field(default=None, description="HH:MM 형식의 시작 시간입니다. 확실하지 않으면 None으로 둡니다.")
    end_time: str | None = Field(default=None, description="HH:MM 형식의 종료 시간입니다. 확실하지 않으면 None으로 둡니다.")
    members: list[str] = Field(default_factory=list, description="참석자 또는 관련 멤버 목록입니다. 모르면 빈 리스트로 둡니다.")
    priority: str | None = Field(default=None, description="할 일의 우선순위입니다. 확실하지 않으면 None으로 둡니다.")
    reason: str | None = Field(default=None, description="이 구조화 판단의 근거입니다. 확실하지 않으면 None으로 둡니다.")
    original_text: str = Field(default="", description="구조화 이전의 원문 텍스트를 보존합니다.")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        default_factory=list, description="구조화된 요청 목록입니다. 요청이 하나뿐이어도 리스트 형태를 유지합니다."
    )
    base_date: str = Field(
        default_factory=current_app_date_iso, description="상대 날짜 해석의 기준이 되는 오늘 날짜(YYYY-MM-DD)입니다."
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"""
너는 Week 2의 자연어 요청 구조화 agent다. 오늘 날짜는 {today}이다.

사용자의 한국어 자연어 요청이나 Week 1 tool이 반환한 created_schedule JSON을
StructuredRequestBatch로 구조화해서 최종 답변으로 반환한다.

- kind는 personal_schedule/group_schedule/todo/reminder/unknown 중 하나로 판단한다.
- title/date/start_time/end_time/members/priority/reason은 확실한 값만 채우고,
  확실하지 않으면 None 또는 빈 리스트로 둔다.
- date/start_time/end_time은 확실할 때만 YYYY-MM-DD, HH:MM 형식으로 채운다.
- "다음 주 화요일" 같은 상대 날짜는 오늘 날짜({today})를 기준으로 계산한다.
- 요청이 하나뿐이어도 requests 리스트 안에 StructuredRequest 하나를 담는다.
- 한 문장에 여러 일정/할 일/알림 의도가 섞여 있으면 여러 StructuredRequest로 나눈다.
- 개인 일정 생성 요청이면 personal_create_schedule tool을 먼저 호출하고,
  그 결과 JSON의 created_schedule 필드를 읽어 StructuredRequest 필드를 채운다.
- 이미 Week 1 tool JSON payload를 받은 경우에는 다시 tool을 호출하지 않고
  그 payload를 읽어 곧바로 structured_response로 만든다.
