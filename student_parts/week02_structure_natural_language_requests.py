...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="요청 종류: personal_schedule(개인 일정), group_schedule(그룹 일정), todo(할 일), reminder(알림), unknown(알 수 없음) 중 하나")
    title: str | None = Field(default=None, description="일정·할 일·알림의 제목 또는 내용 요약")
    date: str | None = Field(default=None, description="날짜 (YYYY-MM-DD 형식, 확실할 때만 채움)")
    start_time: str | None = Field(default=None, description="시작 시각 (HH:MM 형식, 확실할 때만 채움)")
    end_time: str | None = Field(default=None, description="종료 시각 (HH:MM 형식, 확실할 때만 채움)")
    members: list[str] = Field(default_factory=list, description="참석자 또는 관련 멤버 목록, 모르면 빈 리스트")
    priority: str | None = Field(default=None, description="할 일 우선순위 (높음/중간/낮음 등), 해당 없으면 None")
    reason: str | None = Field(default=None, description="종류·날짜·시간 등을 판단한 근거 설명")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="구조화된 요청 목록. 요청이 하나뿐이어도 반드시 리스트 형태로 담는다.")
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜(내일, 다음 주 등) 해석 기준일 (YYYY-MM-DD 형식)")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt([
        *week02_prompt_parts(),
        """최종 답변 규칙:
- 반드시 StructuredRequestBatch 형식으로 최종 답변을 반환하세요.
- 요청이 하나뿐이어도 requests 목록 안에 StructuredRequest 하나를 담으세요.
- personal_create_schedule tool을 호출했다면, 반환된 JSON의 created_schedule 필드를 읽어 title/date/start_time/end_time/members 등 StructuredRequest 필드를 채우세요.
- SQLite 저장, RAG 검색, 외부 멤버 일정 조율은 Week 2에서 하지 않습니다.""",
    ])
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"""당신은 자연어 요청을 구조화된 일정 데이터로 변환하는 Week 2 agent입니다.
오늘 날짜: {current_app_date_iso()}

역할:
- 사용자의 자연어 요청이나 Week 1 tool이 반환한 JSON payload를 StructuredRequestBatch로 변환합니다.
- "내일 오후 3시에 철수랑 회의" 같은 자연어에서 kind/title/date/start_time/end_time/members를 추출합니다.
- 날짜·시간이 "내일", "다음 주 화요일" 같은 상대 표현이면 오늘 날짜({current_app_date_iso()})를 기준으로 YYYY-MM-DD로 변환합니다.
- Week 1 personal_create_schedule tool을 호출해 created_schedule JSON을 받으면, 그 JSON payload를 그대로 읽어 StructuredRequest 필드를 채웁니다. tool을 다시 호출하지 않습니다.
- 확실하지 않은 필드는 억지로 만들지 말고 None 또는 빈 리스트로 두세요.
