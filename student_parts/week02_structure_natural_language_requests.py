...
class StructuredRequest(BaseModel):
    ...
    date: str | None = Field(default=None, description="날짜 (YYYY-MM-DD 형식, 확실할 때만)")
    start_time: str | None = Field(default=None, description="시작 시간 (HH:MM 형식, 확실할 때만)")
    end_time: str | None = Field(default=None, description="종료 시간 (HH:MM 형식, 확실할 때만)")
    members: list[str] = Field(default_factory=list, description="참석자 또는 관련 멤버 목록")
    priority: str | None = Field(default=None, description="할 일 우선순위 (high, medium, low)")
    reason: str | None = Field(default=None, description="요청 종류를 판단한 근거")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="구조화된 요청 목록. 요청이 하나뿐이어도 list 형태를 유지합니다.")
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"[Week 2 요청 구조화 agent]\n"
        f"오늘 날짜: {current_app_date_iso()}\n"
        f"너는 사용자의 자연어 요청을 StructuredRequest 필드(kind, title, date, start_time, end_time, members, priority, reason, original_text)로 구조화하는 agent이다.\n"
        f"kind는 personal_schedule, group_schedule, todo, reminder, unknown 중 하나를 선택한다.\n"
        f"date는 YYYY-MM-DD, start_time과 end_time은 HH:MM 형식으로 확실할 때만 채운다.\n"
