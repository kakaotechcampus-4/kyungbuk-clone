...
def search_nana_memory(
    ...
    context_lines.append("· 저장된 일정/요청:")
    for row in schedule_rows:
        context_lines.append(f"  - {row.get('title', '')}")
    context = "\n".join(context_lines)
    return json_payload(
        {
            "hits": reference_hits,
...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        ...
        "사용자가 새 취향·습관·메모를 알려주면 add_personal_reference로 저장하세요.",
        "취향·습관·메모를 묻는 질문은 되묻기 전에 먼저 search_personal_references로 검색해 답하세요.",
        "저장·등록한 일정·할 일·알림 항목을 찾을 때는 search_saved_requests를 사용하세요.",
        "특정 날짜 범위의 일정을 단순 나열할 때만 기존 일정 나열 tool을 사용하세요.",
        "지난 대화에서 오간 이야기 내용을 되짚는 질문은 search_conversation_messages로 검색하세요.",
        "먼저 알맞은 검색 tool을 호출하고, 그 결과(hits/rows)를 근거로 답하세요.",
    ]
