...
def add_personal_reference_dict(
    ...
    *,
    ...
    ref_item = reference_store.add_personal_reference(
        title=title,
        content=content,
        tags=tags,
    )
    ...
    return {
        "ok": True,
        "reference_backend": ref_item.get("backend", reference_store.backend_info()),
        "reference": ref_item,
    }
...
def search_personal_reference_hits(
    ...
    *,
    ...
    # SearchPersonalReferencesInput의 Field가 default=2, le=20
    safe_k = safe_limit(top_k, default=2, maximum=20)

    raw_hits = reference_store.search_personal_references(query=query, limit=safe_k)
...
def search_saved_request_rows(
    ...
    *,
    ...
    # SearchSavedRequestsInput의 Field가 default=3, le=50
    safe_k = safe_limit(top_k, default=3, maximum=50)

    rows=sqlite_store.search_saved_requests(query=query, limit=safe_k)
...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        ...
        "너는 사용자의 질의를 분석하여 올바른 저장/검색 도구를 고르고 답변해야 한다.\n",
