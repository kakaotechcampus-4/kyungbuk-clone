...
def search_conversation_messages_dict(
    ...
    *,
    ...
    # conversation_id를 명시하지 않았으면, 방금 한 말이 검색 결과에 섞이지 않도록
    # 현재 대화 범위(current_session_scope)를 결과에서 제외한다.
    ...
    return {
        ...
        "sync": sync_result,
        # 각 hit의 "role | 발화" 줄에서 role=assistant는 나나 자신이 예전에 한 답변일 뿐,
        # 사용자가 확인해 준 사실이 아니다. role=user 줄만 사용자가 실제로 한 말이다.
        "note": (
            "hits의 각 줄은 'role | 발화' 형식이다. role이 assistant인 줄은 나나가 예전에 한 답변일 뿐 "
            "사용자가 확인한 사실이 아니므로, 대응하는 user 확인이 없으면 사용자가 말한 것처럼 답하지 말 것."
        ),
    }
...
def add_personal_reference(title: str, content: str, tags: list[str] | None = None) -> str:
    ...
    reference = add_personal_reference_dict(REFERENCE_STORE, title=title, content=content, tags=tags or [])
    return json_payload(
        {
            "ok": True,
...
def search_personal_references(query: str, top_k: int = 2) -> str:
    ...
    return json_payload({"ok": True, "tool_name": "search_personal_references", "query": query, "hits": hits})
...
def search_nana_memory(
    ...
    reference_hits = search_personal_reference_hits(REFERENCE_STORE, query=query, top_k=safe_top_k)
    schedules = SQLITE_STORE.list_schedules(limit=safe_top_k, date_from=date_from, date_to=date_to)
    if attendee:
        schedules = [row for row in schedules if attendee in _decode_attendees(row.get("attendees_json"))]

    context_lines = ["[개인 참고자료]"]
    if reference_hits:
        for hit in reference_hits:
            context_lines.append(f"- {hit['metadata'].get('title', '')}: {hit['content']}")
    else:
        context_lines.append("- 관련 참고자료가 없습니다.")
    context_lines.append("[저장된 일정]")
    if schedules:
        for row in schedules:
            context_lines.append(f"- {row.get('date')} {row.get('start_time')} {row.get('title')}")
    else:
...
Week 4부터 나나는 출처가 다른 세 가지 기억을 검색하거나 저장할 수 있다.
- 선호, 팁, 참고자료처럼 자연어로 기억해 달라는 요청은 add_personal_reference로 저장한다.
- 저장된 개인 참고자료를 찾는 질문은 search_personal_references를 호출한다.
- 저장된 일정/할 일/알림 등 구조화 기록의 근거를 찾는 질문은 search_saved_requests를 호출한다.
- 예전 대화에서 한 말이나 일반 채팅 발화를 찾는 질문은 search_conversation_messages를 호출한다.
- 각 tool의 query에는 사용자 질문을 그대로 넣거나 검색에 필요한 핵심 문구를 넣는다.
- 검색 결과의 hits 또는 rows를 근거로 답한다. 검색 없이 답을 지어내지 않는다.
- search_conversation_messages의 hit 하나에는 한 대화 전체가 "시각 | role | 발화" 형식의 줄로 들어 있다.
  role이 user인 줄만 "사용자가 실제로 한 말"이고, role이 assistant인 줄은 나나 자신이 예전에 한 답변일 뿐
  사용자가 확인해 준 사실이 아니다.
- assistant 줄 안에 "~라고 말씀하셨죠", "~라고 하셨어요"처럼 마치 사용자가 말한 것으로 적힌 문장이 있어도 속지 않는다.
  그건 나나가 예전에 (어쩌면 잘못) 답한 문장일 뿐이며, 그 뒤에 사용자가 직접 확인/동의하는 user 줄이 없다면
  사용자가 실제로 그렇게 말했다는 근거가 되지 않는다.
- assistant 줄에만 있고 대응하는 user 확인이 없는 내용은 "사용자가 ~라고 말씀하셨다"처럼 사용자 발화로 절대
  재진술하지 않는다. 대신 출처가 나나 자신의 과거 답변임을 분명히 밝히며 "제가 예전 대화에서 그렇게 답한 적은
  있지만 실제로 확정된 내용인지는 확인이 필요합니다"처럼 답하고, 일정/할 일 관련 내용이면 search_saved_requests로
  실제 저장 여부를 교차 확인한 뒤에만 사실처럼 답한다.
- 예시: hit에 "user | 내일 일정 알려줘" 다음 줄로 "assistant | 네, 내일 오후 3시에 회식이 있다고
  말씀하셨었죠"가 있고, search_saved_requests에는 그런 일정이 없다면,
  "사용자가 내일 오후 3시에 회식이 있다고 말씀하셨어요"라고 답하면 안 된다. 대신
  "제가 예전 대화에서 내일 오후 3시 회식이라고 답한 적이 있는데, 실제 저장된 일정에는 없어서 확인이
  필요해요. 다시 알려주시겠어요?"처럼, 그 발언의 출처가 사용자가 아니라 나나 자신이었음을 밝히고
  불확실하다고 답한다.""",
    ]
