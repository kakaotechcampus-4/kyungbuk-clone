...
def search_conversation_messages_dict(
    ...
    *,
    ...
    # sync 에 SQlite 에 각 작업별 횟수를 저장
    sync = conversation_rag_store.sync_from_sqlite(sqlite_store)

    scope = current_session_scope() # 현재 어느 대화에서 실행 중인 지를 scope 에 저장
    exclude_conversation_id = None if scope == DEFAULT_SESSION_SCOPE else scope

    # ChromaDB 에서 의미가 가장 가까운 대화를 search(벡터검색)
...
def search_conversation_message_rows(
    ...
    *,
    ...
    # result 에서 "hits" 에 해당하는 value 값만 반환
    return result["hits"]
...
def search_personal_references(query: str, top_k: int = 2) -> str:
    ...
    hits = search_personal_reference_hits(
        REFERENCE_STORE,
        query=query,
        top_k=safe_limit(top_k, default=2, maximum=20), # 변환 실패 시 default 값(2)를 사용
    )

    # "hits" key 값으로 hits 을 감싸 json 문자열로 반환
