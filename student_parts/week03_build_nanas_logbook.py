...
SQLITE_MEMORY_PROMPT = (
    "너는 이제 Week 3 역할도 겸한다. Week 1의 개인 일정은 현재 대화에서만 살아있는 임시 메모리였지만, "
    "Week 3부터 저장한 일정/할 일/알림은 앱 SQLite DB에 남아 새 대화를 열거나 앱을 다시 시작해도 "
    "그대로 유지된다. '내 일정 보여줘', '저장된 일정 알려줘' 같은 조회 요청에는 Week 1의 "
    "personal_list_schedules가 아니라 personal_list_saved_schedules로 답하라. 저장된 요청 원본이 "
    "궁금하면 list_saved_requests 또는 get_saved_request로 조회하라."
)

WEEK03_TOOL_CALL_PROMPT = (
    "개인 일정, 그룹 일정, 할 일, 알림처럼 SQLite에 저장해야 하는 요청을 받으면 다음 순서를 지켜라. "
    "1) 먼저 extract_schedule_request(query=사용자 원문)를 호출해 자연어를 구조화한다. "
    "2) 그 결과 JSON의 structured_request 안에 있는 kind/title/date/start_time/end_time/members/"
    "priority/reason/original_text 값을 그대로 save_structured_request의 인자로 전달해 저장한다. "
    "이 흐름을 쓸 때는 personal_create_schedule을 호출하지 않는다. personal_create_schedule은 "
    "Week 1 호환 전용이라 아직 Week 3 저장 요청에는 사용하지 않는다. "
    "일정 수정이나 삭제 요청을 받으면 아직 지원하지 않는 기능이라고 안내하고 tool을 호출하지 마라."
)
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"오늘 날짜는 {current_app_date_iso()}이다. 이번 주차(Week 3)의 범위는 구조화된 요청을 "
        "SQLite에 저장하고 다시 조회하는 것까지다. 일정 수정, 삭제, 외부 공유 저장소 동기화는 "
        "아직 지원하지 않는다.",
    ]
