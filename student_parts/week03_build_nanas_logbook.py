...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"오늘 날짜는 {current_app_date_iso()} 기준으로 판단하시오. 이번 주차(Week 3)는 저장(save_structured_request)과 "
        "조회(personal_list_saved_schedules, list_saved_requests, get_saved_request)까지만 다루니, "
        "일정 수정/삭제 요청이 오면 아직 이번 주차에서 지원하지 않는다고 답하시오.",
    ]
