...
# Week 3부터는 일정/할 일/알림을 SQLite에 남겨 새 대화에서도 다시 조회할 수 있습니다.
SQLITE_MEMORY_PROMPT = """
Week 3부터 Nana는 현재 대화의 임시 메모리만 보지 않고 SQLite 기록장을 사용한다.
저장된 일정, 할 일, 알림은 새 대화를 열어도 남아 있으므로 사용자가 저장된 내용을 묻거나 수정/삭제하려 하면
기억으로 추측하지 말고 반드시 SQLite 조회 tool을 먼저 사용한다.
"""

# 자연어 요청은 Week 2 구조화 결과로 만든 뒤 Week 3 저장 tool에 넘깁니다.
WEEK03_TOOL_CALL_PROMPT = """
Week 3 tool 호출 규칙:
- 저장 요청은 extract_schedule_request(query=원문)로 구조화한 뒤 save_structured_request에 각 필드를 직접 전달한다.
- 조회 요청은 personal_list_saved_schedules를 호출한다.
- 수정 요청은 personal_list_saved_schedules로 schedule_id 후보를 확인한 뒤 personal_update_saved_schedule을 호출한다.
- 삭제 요청은 personal_list_saved_schedules로 후보를 확인한 뒤 personal_delete_saved_schedules에 schedule_ids 또는 명시 필터를 전달한다.
- SQLite 저장, 조회, 수정, 삭제 결과 JSON의 ok/tool_name/rows/schedules/deleted_count 값을 근거로 답한다.
"""
...
def _delete_saved_schedules(
    *,
    ...
    has_filter = any([schedule_ids, date, title, start_time, time_unspecified, delete_all])
    filters = {
    ...
    if not has_filter:
        return tool_result(
            ...
            ok=False,
            error="삭제 조건이 없습니다. schedule_ids, 날짜, 제목, 시간 중 하나를 지정하세요.",
            filters=filters,
            deleted_count=0,
            deleted=[],
        )
...
def structured_request_from_week01_schedule(schedule: dict[str, Any]) -> SaveStructuredRequestInput:
    ...
    return SaveStructuredRequestInput(
        ...
        end_time=None if schedule.get("end_time") == "미정" else schedule.get("end_time"),
        members=attendees,
    ...
    if updated is None:
        return json_payload(
            tool_result(
                ...
                ok=False,
                error=f"schedule_id={schedule_id} 일정을 찾을 수 없습니다.",
            )
        )
    return json_payload(
        tool_result(
