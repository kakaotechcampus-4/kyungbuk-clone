...
Week 3부터 개인 일정/할 일/알림은 현재 대화의 임시 메모리가 아니라 앱의 SQLite DB에 남는다.
한 번 저장된 기록은 대화가 끝나거나 앱을 다시 시작해도 사라지지 않으므로,
사용자가 "저번에 저장한 일정"처럼 이전 대화를 가리키면 먼저 조회 tool로 DB에서 찾는다.
"""

WEEK03_TOOL_CALL_PROMPT = """
자연어 요청을 SQLite에 남기려면 다음 순서를 지킨다.
1. extract_schedule_request로 자연어(또는 Week 1 tool이 만든 JSON)를 structured_request로 구조화한다.
2. structured_request의 kind/title/date/start_time/end_time/members/priority/reason/original_text 값을
   그대로 save_structured_request tool의 인자로 전달해 저장한다.
3. 저장된 일정을 확인하거나 수정/삭제하기 전에는 personal_list_saved_schedules로 먼저 후보를 조회한다.
4. 수정은 personal_update_saved_schedule, 삭제는 personal_delete_saved_schedules를 사용하고,
   삭제 조건(schedule_ids 또는 날짜/제목/시간)이 전혀 없으면 삭제를 실행하지 않는다.
"""
...
def save_structured_request_payload(
    ...
    *,
    ...
    result = (store or _store()).save_structured_request(save_input.model_dump())
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        WEEK03_TOOL_CALL_PROMPT,
        f"""
너는 Week 3의 기록장 agent다. 오늘 날짜는 {today}이다.

개인 일정 생성 요청은 personal_create_schedule(Week 1 호환 tool)을 호출하면
임시 메모리 생성과 SQLite 저장이 한 번에 처리된다.
그 외의 일정/할 일/알림 요청은 extract_schedule_request로 구조화한 뒤
save_structured_request로 저장한다.

저장된 내용을 보여 달라는 요청에는 personal_list_saved_schedules 또는
list_saved_requests/get_saved_request로 SQLite를 조회해서 답한다.
수정·삭제는 먼저 조회로 대상 schedule_id를 확인한 뒤에만 실행한다.

Week 3에서는 RAG 검색이나 외부 멤버와의 일정 조율은 하지 않는다.
""",
    ]
