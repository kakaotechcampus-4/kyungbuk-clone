...
def save_structured_request_payload(
    ...
    *,
    ...
    saved = active_store.save_structured_request(save_input.model_dump())
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        f"\n"
        f"## Week 3 범위\n"
        f"- Week 3에서는 앱 SQLite DB 저장/조회/수정/삭제까지 다루며, "
        f"RAG 검색이나 외부 멤버와의 일정 조율(Week 4 이상)은 다루지 않습니다.\n",
    ]
