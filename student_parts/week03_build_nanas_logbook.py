...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 Nana는 현재 대화에서만 유지되는 임시 메모리가 아니라 앱 SQLite DB에 남는 '기록장'을 사용한다. "
    "저장된 일정/할 일/알림은 앱을 다시 켜거나 새 대화를 시작해도 그대로 남아 있다. "
    "과거에 저장한 내용을 물으면 기억에만 의존하지 말고 반드시 조회 tool(list_saved_requests, get_saved_request, "
    "personal_list_saved_schedules)로 DB에서 실제 값을 읽어 답한다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "일정/할 일/알림을 저장할 때는 먼저 extract_schedule_request(query=사용자 원문)로 자연어를 StructuredRequest로 "
    "구조화한 뒤, 그 structured_request의 kind/title/date/start_time/end_time/members/priority/reason/original_text "
    "필드를 그대로 save_structured_request 인자로 넘겨 저장한다. "
...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        if not isinstance(value, dict):
            return value
        # extract_schedule_request 결과나 예전 trace는 실제 필드를 wrapper 안에 감싸 둔다.
        for key in ("structured_request", "payload"):
            inner = value.get(key)
            if isinstance(inner, dict):
                merged = dict(inner)
                source_id = value.get("source_schedule_id")
                if source_id is not None and "source_schedule_id" not in merged:
                    merged["source_schedule_id"] = source_id
                return merged
        # ok/tool_name/base_date 같은 wrapper 필드는 저장 스키마에 없으므로 걷어낸다.
        wrapper_keys = {"ok", "tool_name", "base_date"}
        if wrapper_keys & value.keys():
            return {k: v for k, v in value.items() if k not in wrapper_keys}
        return value
...
def structured_request_from_week01_schedule(schedule: dict[str, Any]) -> SaveStructuredRequestInput:
    ...
    if end_time in ("", "미정"):
        end_time = None
    # 참석자가 있으면 그룹 일정, 나 혼자면 개인 일정으로 본다(Week 2 kind 판정 규칙과 동일).
...
def personal_list_saved_schedules(
    ...
    effective_kind = kind if kind is not None else "personal_schedule"
...
def personal_update_saved_schedule(
    ...
    # None인 필드는 "수정하지 않음" — store.update_schedule이 기존 값을 유지한다.
    ...
    if result is None:
        return json_payload(
            tool_result(
                ...
                error="해당 schedule_id를 찾을 수 없습니다.",
            )
        )
    return json_payload(
        tool_result(
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        "Week 2에서 만든 StructuredRequest 구조화 결과는 Week 3에서 앱 SQLite DB에 저장하고 다시 조회/수정/삭제하는 "
        "기록장의 입력으로 이어진다. 구조화에서 멈추지 말고 저장 흐름까지 완성한다.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"오늘 날짜는 {current_app_date_iso()}이며, '오늘/내일/다음 주' 같은 상대 표현은 이 날짜를 기준으로 계산한다. "
        "Week 3에서는 구조화한 요청을 SQLite에 저장하고, 저장된 일정/요청을 조회·수정·삭제하는 단계까지 수행한다. "
        "저장 요청에는 save_structured_request, 저장 기록 조회에는 list_saved_requests/get_saved_request, "
        "일정 목록에는 personal_list_saved_schedules, 수정에는 personal_update_saved_schedule, "
        "삭제에는 personal_delete_saved_schedules를 고른다. 조회 결과는 DB에서 읽은 실제 값을 근거로 답하고 지어내지 않는다.",
    ]
