...
SQLITE_MEMORY_PROMPT = (
    "이번 주차부터 개인 일정·할 일·알림은 현재 대화 안에서만 유지되는 임시 메모리가 아니라 "
    "앱 SQLite DB에 영구히 저장되는 기록장이다. 저장된 기록은 앱을 다시 시작하거나 새 대화를 "
    "열어도 그대로 남아 있으므로, 조회/수정/삭제 요청은 현재 대화에서 만든 일정만이 아니라 "
    "DB에 저장된 모든 기록을 대상으로 처리한다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "저장 흐름은 다음 순서를 따른다. "
    "1) 사용자의 자연어 저장 요청은 먼저 extract_schedule_request(query=원문)를 호출해 "
    "Week 2 StructuredRequest로 구조화한다. "
    ...
    "조회는 저장 원본 기록이면 list_saved_requests/get_saved_request를, 저장된 일정 목록이면 "
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    if isinstance(value, str):
        ...
        # 순수 자연어는 Week 2 bridge로 먼저 구조화한 뒤 저장 입력으로 검증합니다.
        return SaveStructuredRequestInput.model_validate(extract_structured_request(text))
    raise RuntimeError(f"저장 입력으로 쓸 수 없는 형식입니다: {type(value).__name__}")
...
def save_structured_request_payload(
    ...
    *,
    ...
    active_store = store if store is not None else _store()
    result = active_store.save_structured_request(save_input.model_dump(exclude_none=True))
...
def structured_request_from_week01_schedule(schedule: dict[str, Any]) -> SaveStructuredRequestInput:
    ...
    def _clean_time(value: Any) -> str | None:
        # Week 1 기본값 '미정'이나 빈 문자열은 저장 스키마에서 None으로 둡니다.
        if value in (None, "", "미정"):
            return None
        return str(value)
...
def list_saved_requests(
    ...
    return json_payload(tool_result("list_saved_requests", rows=rows, filters=filters))
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        (
            "너는 이제 구조화 결과를 실제 앱 DB에 저장하고 다시 꺼내 쓰는 Week 3 기록장 agent다. "
            "Week 2까지는 '저장/조율을 하지 않는다'고 안내했지만, 이번 주차부터는 그 제한을 해제하고 "
            "구조화한 요청을 SQLite에 저장·조회·수정·삭제한다. 이 지시가 앞선 Week 2의 '저장하지 않는다' "
            "지시보다 우선한다."
        ),
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        (
            ...
            "일정/할 일/알림을 앱에 남겨야 하는 요청에는 extract_schedule_request → save_structured_request "
            "저장 흐름을, 저장된 기록을 보여주거나 고치거나 지우는 요청에는 조회/수정/삭제 tool을 선택한다. "
            "Week 3에서는 RAG 검색이나 외부 멤버 일정 조율은 다루지 않고, 앱 DB 기록장 관리에 집중한다."
        ),
    ]
