...
2. 그 결과의 kind/title/date/start_time/end_time/members/priority/reason/original_text 필드를
   save_structured_request 도구 인자로 그대로 전달해 SQLite에 저장한다.

저장된 일정/요청을 조회할 때는 personal_list_saved_schedules(일정 목록),
list_saved_requests(구조화 요청 원본 목록), get_saved_request(단건 조회) 중 알맞은 도구를 사용한다.

저장된 일정을 수정하거나 삭제할 때는 먼저 personal_list_saved_schedules로 대상 schedule_id를 확인한 뒤,
personal_update_saved_schedule 또는 personal_delete_saved_schedules에 그 schedule_id를 전달한다.
조건 없이 전체 삭제를 요청받은 경우가 아니면 delete_all을 사용하지 않는다.
"""
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        구조화만 하고 멈추지 않고, 반드시 저장까지 이어지는 tool 호출을 완료한 뒤 사용자에게 한국어로 답한다.
        """,
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        ...
        일정/할 일/알림을 저장해 달라는 요청은 참석자 유무와 관계없이
