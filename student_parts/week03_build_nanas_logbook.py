...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 개인 일정/할 일/알림은 대화가 끝나도 사라지지 않는 앱 SQLite DB에 저장된다. "
    "새 대화를 시작하거나 앱을 다시 켜도 이전에 저장한 일정을 personal_list_saved_schedules로 다시 조회할 수 있다. "
    "Week 1의 personal_create_schedule은 이제 SQLite 저장까지 함께 수행하는 Week 3 tool로 대체된다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "자연어 저장 요청이 들어오면 먼저 extract_schedule_request로 구조화한 뒤, 그 결과의 "
    "structured_request 필드 값을 save_structured_request 인자로 그대로 전달해 SQLite에 저장한다. "
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        WEEK03_TOOL_CALL_PROMPT,
        (
            f"오늘 날짜는 {current_app_date_iso()}이다. 이번 주차(Week 3)에서는 SQLite 저장/조회/수정/삭제까지만 다루고, "
            "RAG 문서 검색이나 외부 멤버 일정 조율은 다루지 않는다."
        ),
    ]
