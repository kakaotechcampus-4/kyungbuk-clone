...
[Week 3] 이제 개인 일정/할 일/알림은 대화창의 임시 기억이 아니라 앱 SQLite DB에 저장된다.
새 대화를 시작하거나 앱을 다시 실행해도 저장된 내용은 사라지지 않으므로, "내 일정 보여줘" 같은
조회 요청에는 대화 기록에만 의존하지 말고 반드시 personal_list_saved_schedules 또는
list_saved_requests/get_saved_request로 DB를 다시 조회한 뒤 답하세요."""

WEEK03_TOOL_CALL_PROMPT = """
[Week 3] 일정/할 일/알림을 저장해야 하는 요청이면 다음 순서를 지키세요.
1) extract_schedule_request를 호출해 자연어(또는 Week 1 tool JSON)를 structured_request로 구조화하세요.
2) 그 structured_request의 kind/title/date/start_time/end_time/members/priority/reason/original_text 값을
   그대로 save_structured_request 인자로 전달해 SQLite에 저장하세요.
저장된 내용을 확인하거나 "내 일정 보여줘" 같은 질문에는 personal_list_saved_schedules를 사용하고,
구조화 원본 기록을 그대로 확인해야 하면 list_saved_requests 또는 get_saved_request를 사용하세요."""
...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        if "structured_request" in value:
            inner = value["structured_request"]
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    if isinstance(value, str):
        looks_like_json = value.strip().startswith("{")
        if looks_like_json:
...
def _delete_saved_schedules(
    *,
    ...
    has_id_filter = bool(schedule_ids)
    has_other_filter = bool(date) or bool(title) or bool(start_time) or time_unspecified
    has_condition = delete_all or has_id_filter or has_other_filter
    if not has_condition:
        return tool_result(
            ...
            filters=filters,
            deleted=[],
            reason="삭제 조건이 없어 거부되었습니다. schedule_ids 또는 날짜/제목/시간 필터, delete_all 중 하나가 필요합니다.",
        )
...
def personal_update_saved_schedule(
    ...
    if result is None:
        return json_payload(
            tool_result("personal_update_saved_schedule", ok=False, updated_schedule=None, shared_sync=None)
        )
    return json_payload(
        tool_result(
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        "[Week 3] Week 2에서 만든 StructuredRequest는 이제 대화 응답에서 끝나지 않고, "
        "extract_schedule_request로 구조화한 뒤 save_structured_request로 SQLite 앱 DB에 저장하는 것까지가 이번 주차 범위다.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"오늘 날짜는 {current_app_date_iso()} 기준으로 판단하시오. 이번 주차(Week 3)는 저장(save_structured_request), "
        "조회(personal_list_saved_schedules, list_saved_requests, get_saved_request), "
        "수정(personal_update_saved_schedule), 삭제(personal_delete_saved_schedules)까지 다룬다. "
        "수정/삭제 전에는 먼저 personal_list_saved_schedules로 대상 schedule_id를 확인한 뒤 호출하시오. "
        "삭제는 조건(schedule_ids 또는 날짜/제목/시간) 없이 delete_all만으로 요청받아도 사용자에게 다시 확인하시오.",
    ]
