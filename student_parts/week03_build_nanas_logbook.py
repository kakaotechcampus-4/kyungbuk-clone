...
일정/할 일/알림 저장이 필요한 요청은 먼저 extract_schedule_request()를 호출해 StructuredRequest로 구조화하세요.
그 결과의 kind/title/date/start_time/end_time/members/priority/reason/original_text 값을 save_structured_request의 같은 이름 인자로 전달하고, 저장하세요.
"""
...
def save_structured_request_payload(
    ...
    *,
    ...
    saved = selected_store.save_structured_request(
        structured_request.model_dump(exclude_none=True)
    )
...
def structured_request_from_week01_schedule(schedule: dict[str, Any]) -> SaveStructuredRequestInput:
    ...
    return SaveStructuredRequestInput(
        ...
        members=attendees,
        reason=(
            "Week 3 personal_create_schedule 호환 도구가 Week 1 임시 일정을 저장 입력으로 변환했습니다."
        ),
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        (
            "Week 2가 구조화한 결과는 Week 3부터 SQLite에 저장해야 다음 대화에서도 남습니다. "
            "구조화 후에는 반드시 저장 tool을 호출하세요."
        ),
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
