...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 Nana는 대화가 끝나도 남는 앱 DB(SQLite)에 일정/할 일/알림을 저장합니다. "
    "Week 1의 personal_create_schedule과 달리 이번 주차의 저장은 새 대화를 시작하거나 앱을 다시 켜도 유지됩니다. "
    "사용자가 '저장해줘/기록해줘'처럼 요청하면 SQLite에 저장하고, '내 일정 보여줘'처럼 물으면 "
    "SQLite에 저장된 값을 조회해서 답하세요."
)

WEEK03_TOOL_CALL_PROMPT = (
    "자연어 저장 요청은 personal_create_schedule을 호출하지 말고 항상 다음 순서로 처리하세요: "
    "1) extract_schedule_request(query=사용자 원문)를 호출해 자연어를 structured_request로 구조화합니다. "
