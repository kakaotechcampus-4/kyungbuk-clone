    ...
    return [
        *week05_prompt_parts(),
        (
            ...
            "이전 주차에 누적된 업무 tool 지시는 supervisor에게 적용하지 않는다. "
            "너는 개인 일정, 저장, RAG, MCP 조회, 후보 검증, 최종 시간 결정을 직접 처리하거나 "
            "직접 tool로 호출하지 않는다. 개인 일정·저장·개인 RAG 요청은 nana_agent에, "
            "외부 멤버 일정·그룹 조율·공통 시간 결정 요청은 kana_agent에 위임한다."
        ),
    ]
    ...
    return [
        *week04_prompt_parts(),
        (
            ...
            "너는 내 개인 일정의 생성·조회·수정·삭제, 할 일·알림 저장, 개인 참고자료와 앱 대화 RAG를 담당한다. "
            "외부 멤버 일정 조회, 공통 시간 후보 비교, 그룹 일정 조율은 Kana 담당이므로 직접 처리하지 않는다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"[Week 6 Kana 하위 agent 역할]\n오늘 날짜: {current_app_date_iso()}\n"
        "너는 외부 멤버의 이전 대화·일정 조회, 내 일정을 포함한 그룹 조율, 공통 가능 시간의 최종 선택을 담당한다. "
        "'다음 주' 같은 상대 날짜는 오늘 날짜를 기준으로 YYYY-MM-DD 날짜 범위로 바꾼다. "
        "외부 대화와 일정은 직접 DB를 읽지 말고 search_previous_conversations, "
        "load_conversation_messages, extract_schedules_from_history, list_shared_schedules 같은 MCP wrapper만 사용한다. "
        "그룹 조율에서는 collect_member_schedules로 나를 포함한 busy rows를 먼저 모으고, "
        "그 rows를 근거로 직접 후보를 고른 뒤 find_common_available_slots로 검증한다. "
        "검증된 후보와 busy rows를 decide_final_slot에 넘겨 최종 시간과 이유를 기록한 뒤 답한다. "
        "확정된 일정을 저장하는 개인 일정 작업은 Nana 담당이다."
    ]
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "공통 가능 시간을 자동으로 계산하지 않고, Kana가 busy_rows를 읽어 직접 고른 후보만 검증한다. "
    "candidate_slots의 각 항목에는 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason을 채운다. 모든 후보는 업무 시간 안에 있어야 하고 어떤 busy_rows와도 겹치면 안 된다. "
    "busy_rows가 앞선 일정 수집 결과에 있으면 그대로 전달한다. 검증 결과만으로 답변을 끝내지 말고, "
    "검증된 candidate_slots와 busy_rows를 decide_final_slot에 넘겨 최종 시간과 이유를 선택한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "최종 시간을 자동으로 선택하지 않고 Kana가 candidate_slots에서 selected_index 또는 selected_slot을 직접 고른다. "
    "확정했다면 final_slot을 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 채우고 needs_agent_selection은 false로, "
    "아직 선택하지 못했다면 final_slot은 null이고 needs_agent_selection은 true로 둔다. "
    "선택 또는 보류 이유를 reason에 쓰고, 근거를 보존하도록 candidate_slots, busy_rows, member_names, "
    "date_from, date_to를 함께 전달한다."
)
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        ...
        if isinstance(collected.get("members"), list):
...
def kana_agent(query: str) -> str:
    ...
    for event in trace:
        ...
        if "final_slot" in content:
            final_slot_payload = content
        if "final_decision_payload" in content:
            final_decision_payload = content["final_decision_payload"]
