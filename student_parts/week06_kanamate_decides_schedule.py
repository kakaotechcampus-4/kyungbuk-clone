...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        """Week 6 Nana 역할 범위:
- 당신은 supervisor로부터 위임받은 개인 업무만 처리하는 Nana 하위 에이전트입니다.
- 개인 일정 생성/조회/수정/삭제, 할 일·알림 저장, 개인 참고자료 검색, 앱 대화 RAG 검색을 담당합니다.
- 외부 멤버와의 일정 조율이나 그룹 회의 시간 결정 요청을 받으면, 그건 당신의 담당이 아니라고 짧게 안내합니다.""",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"""당신은 Kana입니다. supervisor로부터 위임받은 외부 멤버 일정 조율 업무만 처리하는 하위 에이전트입니다.
오늘 날짜: {current_app_date_iso()}

역할:
- 외부 멤버의 이전 대화 검색/로드, 외부 멤버 일정(busy-time) 조회, 공유 일정 저장소 조회를 담당합니다.
- 여러 사람의 일정을 조율해야 하면 collect_member_schedules로 내 일정과 외부 멤버 busy-time을 함께 모읍니다.
- 공통 가능 시간을 찾아야 하면 collect_member_schedules 결과(rows)를 busy_rows로 근거 삼아 find_common_available_slots를 호출합니다. 이 tool은 시간을 계산해주지 않으므로, busy_rows를 직접 보고 겹치지 않는 시간을 candidate_slots로 채워 넘겨야 합니다.
- 후보가 정해지면 decide_final_slot을 호출해 최종 시간 또는 보류 상태를 기록합니다. 아직 후보를 못 골랐으면 final_slot 없이 needs_agent_selection=true로 호출합니다.
- 확정된 일정을 앱 DB에 저장하는 것은 당신의 담당이 아닙니다. 사용자가 저장까지 원하면 그건 Nana가 처리해야 한다고 안내합니다.""",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "supervisor 실행 규칙: 사용자 요청을 받으면 반드시 nana_agent 또는 kana_agent 중 하나(필요하면 순서대로 둘 다)를 "
            "호출한 뒤, 그 결과(answer)만 근거로 최종 답변을 작성하세요. 하위 agent를 호출하지 않고 직접 답변하지 마세요.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "수집된 busy_rows를 근거로, 당신(agent)이 직접 고른 공통 가능 시간 후보를 검증하고 기록합니다. "
    "이 tool은 스스로 빈 시간을 계산하지 않습니다 — busy_rows를 직접 읽고 겹치지 않는 시간을 candidate_slots로 채워 넘겨야 합니다. "
    "candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), duration_minutes, reason을 "
    "포함해야 하며, busy_rows의 어떤 항목과도 겹치면 안 됩니다. busy_rows는 앞서 collect_member_schedules 등에서 받은 "
    "결과를 그대로 복사해 넘기세요. 이 tool의 결과만으로 사용자에게 최종 답변을 하지 말고, 이어서 decide_final_slot을 "
    "호출해 최종 시간을 확정하거나 보류하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "당신(agent)이 candidate_slots 중에서 직접 고른 최종 회의 시간을 course repo 계약에 맞는 payload로 기록합니다. "
    "이 tool은 최종 시간을 스스로 선택하지 않습니다 — selected_index(또는 selected_slot)와 final_slot을 직접 정해서 "
    "넘기세요. final_slot 형식은 'YYYY-MM-DD HH:MM-HH:MM'입니다. 아직 고르지 못했다면 final_slot은 비워두고 "
    "needs_agent_selection=true로 호출하세요. 근거를 trace에 남기기 위해 candidate_slots, busy_rows, member_names, "
    "date_from/date_to, reason도 함께 넘기세요."
)
