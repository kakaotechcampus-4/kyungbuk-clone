    ...
    return [
        *week05_prompt_parts(),
        (
            ...
            "nana_agent에게 위임하세요. "
            "\"팀 회의\", \"우리 팀\", \"다 같이\"처럼 실제 외부 인물 이름 없이 막연하게 여러 명을 "
            "가리키는 표현만 있는 요청도 개인 업무이니 nana_agent에게만 위임하세요. "
            "철수, 영희처럼 구체적인 외부 인물 이름이 실제로 언급된 이전 발화·일정 조회, 여러 사람의 "
            "공유 일정 저장소 조회, 여러 사람의 공통 가능 시간 조율처럼 실제 외부 멤버 정보가 필요한 "
            "업무만 kana_agent에게 위임하세요. "
            "요청이 개인 업무인지 외부/그룹 업무인지 애매하면, 구체적인 외부 인물 이름이 실제로 언급됐는지를 기준으로 판단하세요. "
    ...
    return [
        *week04_prompt_parts(),
        (
            ...
            "실제 외부 인물 이름 대신 \"팀 회의\", \"우리 팀\", \"다 같이\"처럼 막연한 표현만 있는 "
            "요청은 평범한 개인 일정이니 그대로 만들어·조회·삭제하세요 — extract_schedule_request가 "
            "kind를 group_schedule로, members를 [\"팀\"]처럼 실제 사람 이름이 아닌 값으로 분류해도 "
            "그 결과만 보고 거절하면 안 됩니다. "
            "철수, 영희처럼 구체적인 외부 인물 이름이 실제로 언급돼서 그 사람의 일정·발화 조회나 "
            "여러 사람의 공통 가능 시간 조율처럼 당신이 접근할 수 없는 정보가 정말 필요할 때만 "
            "직접 처리하지 말고, 근거 없는 답을 지어내지 말고 담당이 아니라고 한두 문장으로 짧게 알리세요. "
