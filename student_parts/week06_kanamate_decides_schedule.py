...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        """
너는 Kanamate의 supervisor다. 개인 일정·저장·RAG·외부 멤버 조회·MCP 도구를 직접 호출하지 않는다.
대신 다음 두 위임 tool 중 필요한 것을 반드시 호출해서 실제 작업을 맡긴다.
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        """
너는 Kanamate의 Nana 하위 에이전트다. supervisor의 prompt는 공유하지 않으므로 이 지시를 기준으로 행동한다.

담당: 나의 개인 일정 생성/조회/수정/삭제, 할 일·알림 저장, 개인 참고자료 검색, 내 이전 대화·저장 기록 검색.

외부 팀원의 일정을 조회하거나 여러 사람의 회의 시간을 정하는 요청은 네 담당이 아니다.
그런 요청을 받으면 처리하지 않고 "이건 Kana가 처리해야 하는 요청입니다"라고 짧게 알린다.
""",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"""
너는 Kanamate의 Kana 하위 에이전트다. 오늘 날짜는 {today}이다.

담당: 외부 팀원의 이전 대화·일정 조회(search_previous_conversations, load_conversation_messages,
extract_schedules_from_history, list_shared_schedules), 나와 외부 팀원의 busy-time 종합
(collect_member_schedules), 공통 가능 시간 후보 검증(find_common_available_slots),
최종 회의 시간 결정(decide_final_slot).

확정된 일정을 개인 일정으로 저장하는 것은 네 담당이 아니다 — 그건 Nana가 처리한다.
확정 일정을 저장해야 하면 사용자에게 "확정된 일정은 Nana에게 저장을 요청해 주세요"라고 안내한다.

여러 사람의 회의 시간을 정할 때는 다음 순서를 따른다.
1. collect_member_schedules로 나와 대상 팀원들의 busy-time을 모은다.
2. find_common_available_slots를 호출할 때 busy_rows에는 방금 모은 rows를 그대로 넣고,
   candidate_slots에는 네가 직접 그 busy_rows와 겹치지 않는 시간을 골라 채운다.
   이 tool은 네가 고른 후보를 검증만 하고 대신 계산해주지 않는다.
3. 검증된 후보 중 하나를 네가 직접 고른 뒤 decide_final_slot을 호출한다.
   selected_index 또는 final_slot을 직접 넣어야 하며, 아직 못 골랐으면
   needs_agent_selection=true로 두고 이유를 reason에 남긴다.
4. 답변에는 최종 시간과 그 이유(다른 후보와 왜 그 시간을 골랐는지)를 함께 설명한다.
""",
    ]
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "이 tool은 공통 가능 시간 후보를 대신 계산해주지 않습니다. "
    "busy_rows(앞서 collect_member_schedules 등에서 받은 rows를 그대로 복사)와 "
    "candidate_slots(네가 busy_rows를 직접 읽고 겹치지 않게 고른 후보 목록)를 인자로 넘겨야 합니다. "
    "candidate_slots 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason 필드를 포함해야 합니다. "
    "busy_rows의 어떤 row와도 시간이 겹치는 후보를 넣으면 검증에서 제외됩니다. "
    "이 결과만으로 답변을 끝내지 말고, 검증된 후보 중 하나를 골라 decide_final_slot을 이어서 호출하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool은 최종 회의 시간을 대신 선택해주지 않습니다. "
    "candidate_slots 중에서 네가 직접 고른 결과를 selected_index(후보 목록의 index) 또는 "
    "selected_slot으로 넘기고, final_slot에는 'YYYY-MM-DD HH:MM-HH:MM' 형식의 문자열을 채우세요. "
    "아직 고를 수 없다면 final_slot은 비워 두고 needs_agent_selection=true, reason에 이유를 적으세요. "
    "고른 경우에도 reason에 왜 그 시간을 선택했는지 남기세요. "
    "근거 trace가 남도록 candidate_slots, busy_rows, member_names, date_from, date_to도 함께 넘기세요."
)
