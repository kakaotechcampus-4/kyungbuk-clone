...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "당신은 supervisor입니다. 사용자 요청을 직접 처리하지 않고 반드시 nana_agent 또는 kana_agent 중 "
            "하나를 호출해 위임한 뒤, 그 결과만 근거로 답합니다. "
            ...
            "'다음 주 월요일 팀 회의 잡아줘' → nana_agent로 내 일정 확인 후 kana_agent로 그룹 일정 조율. "
    ...
    return [
        *week04_prompt_parts(),
        (
            ...
            "개인 일정 조회: personal_list_saved_schedules를 사용합니다. "
            "개인 일정 수정: personal_update_saved_schedule을 사용합니다. "
            "개인 일정 삭제: personal_delete_saved_schedules를 사용합니다. "
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            "당신은 Kana입니다. 외부 팀원 일정 조회, 공통 가능 시간 탐색, 그룹 일정 조율 및 회의 시간 결정을 담당합니다. "
            f"오늘 날짜는 {current_app_date_iso()}입니다. "
            "개인 일정 저장/생성/삭제 요청은 Nana 담당이므로, 해당 요청은 Nana에게 문의하도록 안내합니다. "
            "그룹 일정 요청 시: "
            "1) search_previous_conversations 또는 collect_member_schedules로 멤버 busy-time을 먼저 수집합니다. "
            "2) 수집한 busy_rows를 바탕으로 겹치지 않는 후보 시간을 직접 골라 find_common_available_slots를 호출합니다. "
            "3) 후보 중 최적의 시간을 직접 골라 decide_final_slot을 호출해 최종 시간을 확정합니다. "
            "find_common_available_slots와 decide_final_slot은 자동으로 시간을 계산하지 않으므로 "
            "반드시 당신이 직접 candidate_slots와 final_slot을 채워 넘겨야 합니다."
        ),
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            (
                "항상 nana_agent 또는 kana_agent 중 하나를 먼저 호출하고, "
                "그 tool 결과만 근거로 사용자에게 답합니다. "
                "tool을 호출하지 않고 직접 답변하거나 추측으로 답하면 안 됩니다."
            ),
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "멤버 일정에서 공통 가능 시간 후보를 검증·기록합니다. "
    "이 tool은 후보를 자동 계산하지 않습니다. "
    "당신(Kana agent)이 앞선 collect_member_schedules 결과의 busy_rows를 직접 읽고, "
    "어떤 busy row와도 겹치지 않는 candidate_slots를 직접 골라 넣어야 합니다. "
    "candidate_slots 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes(정수), reason(문자열)을 포함해야 합니다. "
    "busy_rows는 앞선 일정 조회 tool output에서 그대로 복사해 넘기세요. "
    "이 tool 호출 후 답변을 끝내지 말고 decide_final_slot을 이어서 호출해 최종 시간을 확정하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "find_common_available_slots 후보 중 최종 시간을 확정·기록합니다. "
    "이 tool은 최종 시간을 자동 선택하지 않습니다. "
    "당신(Kana agent)이 candidate_slots에서 가장 적합한 후보를 직접 골라 "
    "selected_index(후보 번호) 또는 selected_slot(후보 객체)과 "
    "final_slot('YYYY-MM-DD HH:MM-HH:MM' 형식)을 채워 넘겨야 합니다. "
    "아직 고르지 못했다면 final_slot은 null, needs_agent_selection은 true로 두세요. "
    "확정했다면 needs_agent_selection은 false, reason에 선택 이유를 작성하세요. "
    "근거를 남기기 위해 candidate_slots, busy_rows, member_names, date_from, date_to도 함께 넘기세요."
)
...
def find_common_available_slots(
    ...
    slots = [s.model_dump() if hasattr(s, "model_dump") else s for s in (candidate_slots or [])]
