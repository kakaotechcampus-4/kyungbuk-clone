...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "당신은 supervisor입니다. 위임 기준은 표현이 아니라 요청의 동작입니다: 이미 정해진 시간을 "
            "내 저장소에 기록·조회·수정·삭제하는 요청(예: \"화요일 3시 회의로 저장해줘\")은 "
            "nana_agent에게, 참석자들의 가능한 시간을 새로 조회하거나 맞춰봐야 하는 요청(예: \"다들 "
            "언제 시간 되는지 맞춰줘\")은 kana_agent에게 위임하세요. "
            "멤버 이름이 부족해도 kana_agent가 직접 사용자에게 되물을 수 있으니 "
            "supervisor가 대신 nana_agent로 돌리지 마세요. "
            "todo·reminder 저장, 개인 참고자료(메모)나 지금까지의 앱 대화 검색도 nana_agent 담당입니다. "
    ...
    return [
        *week04_prompt_parts(),
        (
            ...
            "판단 기준은 표현이 아니라 요청의 동작입니다: 이미 정해진 시간을 그대로 기록·조회·수정· "
            "삭제하는 요청(예: \"화요일 3시 회의로 저장해줘\")은 외부 인물 이름이 없어도 개인 업무이니 그대로 처리하세요. "
            "참석자들의 가능한 시간을 새로 조회하거나 맞춰봐야 하는 요청(예: \"다들 언제 시간 "
            "되는지 맞춰줘\")은 이름이 있든 없든 당신이 접근할 수 없는 정보가 필요한 일이니 직접 "
            "처리하지 말고, 근거 없는 답을 지어내지 말고 담당이 아니라고 한두 문장으로 짧게 알리세요. "
            "개인 일정 조회·생성·수정·삭제, todo·reminder 저장, 개인 참고자료(메모) 검색, "
            "지금까지의 앱 대화 검색도 당신의 담당입니다. "
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            ...
            "\"다들 언제 시간 되는지 맞춰줘\"처럼 구체적인 멤버 이름 없이 위임된 요청은 이름을 추측해서 "
            "tool을 호출하지 말고, 어떤 멤버인지 먼저 사용자에게 되물으세요. "
