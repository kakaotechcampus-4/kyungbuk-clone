...
def nana_prompt_parts() -> list[str]:
    return [
        *week04_prompt_parts(),
        "[Nana 하위 에이전트 역할]",
        "너는 개인 일정/저장/RAG를 전담하는 에이전트다.",
        "개인 일정과 할 일을 저장, 조회, 삭제하거나 과거의 내 메모를 찾아라.",
        "그룹 일정 조율이나 외부 멤버 일정 검색 요청이 들어오면 '저는 담당이 아닙니다'라고 답변하라.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "반드시 nana_agent 또는 kana_agent 중 하나의 도구를 호출해 결과를 얻고, 그 결과를 근거로만 최종 답변을 작성하라.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "수집된 일정에서 공통 빈 시간을 찾습니다.\n"
    "주의: 이 도구는 알아서 빈 시간을 계산해주지 않습니다! LLM이 busy_rows를 직접 읽고 빈 시간을 찾아 "
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    "최종 그룹 회의 시간을 확정합니다.\n"
    "주의: 이 도구는 알아서 최종 시간을 골라주지 않습니다. LLM이 후보 중 하나를 골라 selected_index (또는 selected_slot)로 "
    "넘기고, final_slot 텍스트('YYYY-MM-DD HH:MM-HH:MM')를 직접 채워야 합니다.\n"
    "아직 확정하기 어렵다면 final_slot을 null로 두고 needs_agent_selection=true로 두세요.\n"
    "근거를 위해 candidate_slots, busy_rows, member_names, date_from/date_to 인자를 모두 넘겨주세요."
)
...
def find_common_available_slots(
    ...
    res = find_common_available_slots_dict(
        member_names, date_from, date_to, duration_minutes, workday_start, workday_end, limit,
        busy_rows, candidate_slots, llm_reason
    )
