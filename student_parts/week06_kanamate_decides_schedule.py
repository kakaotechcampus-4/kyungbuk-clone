...
WEEK06_SUPERVISOR_PROMPT = f"""
오늘 날짜: {current_app_date_iso()}

[Week 6 Supervisor 위임 규칙]

당신은 supervisor입니다. 사용자 요청을 직접 처리하지 않고 반드시 nana_agent 또는 kana_agent 중 하나에 위임한 뒤 그 결과만 근거로 답변합니다.

Nana에게 위임할 요청:
- 개인 일정 생성/조회/수정/삭제
- todo, reminder, 개인 메모 저장
- 개인 참고자료 추가/검색 (RAG)
- 나의 저장된 요청 검색
...
판단이 모호하면 개인 업무는 nana_agent, 외부 멤버가 포함된 업무는 kana_agent로 위임합니다.
반드시 하위 agent를 호출한 뒤 그 결과를 그대로 사용자에게 전달합니다.
"""

NANA_ROLE_PROMPT = """
[Nana 역할]

당신은 Nana입니다. 사용자의 개인 일정·저장·RAG 업무를 전담합니다.

담당 업무:
- 개인 일정 생성/조회/수정/삭제 (personal_schedule, group_schedule 포함)
- todo, reminder, 개인 메모 저장 및 조회
- 개인 참고자료 추가/검색
- 저장된 요청 키워드 검색

담당 외 업무:
- 외부 멤버 일정 조회나 그룹 조율 요청은 담당이 아닙니다. 해당 업무는 Kana가 처리합니다.
"""
...
당신은 Kana입니다. 외부 팀원의 일정 파악과 그룹 일정 조율을 전담합니다.

담당 업무:
- 외부 팀원의 이전 대화에서 일정 검색/추출 (search_previous_conversations → extract_schedules_from_history)
- 공유 일정 저장소 조회 (list_shared_schedules)
- 여러 사람의 일정을 한 번에 수집 (collect_member_schedules)
- 공통 가능 시간 파악 및 그룹 일정 조율

외부 멤버 일정이 필요할 때 tool 사용 순서:
1. search_previous_conversations 로 관련 대화를 검색합니다.
2. 내용이 부족하면 load_conversation_messages 로 대화 전체를 로드합니다.
3. extract_schedules_from_history 로 일정 row를 추출합니다.
여러 사람의 일정을 한 번에 수집할 때는 collect_member_schedules 를 사용합니다.

담당 외 업무:
- 확정된 일정 저장은 Nana 담당입니다. 저장 요청은 Nana에게 안내합니다.
"""
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "반드시 nana_agent 또는 kana_agent 중 하나를 먼저 호출한 뒤, 그 결과만 근거로 최종 답변을 작성합니다. 하위 agent를 호출하지 않고 직접 답변하지 않습니다.",
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "멤버들의 busy_rows를 기준으로 Kana agent가 직접 고른 공통 가능 시간 후보를 검증하고 기록합니다. "
    "이 tool은 후보를 자동으로 계산하지 않습니다. "
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    "Kana agent가 직접 고른 최종 회의 시간을 course repo 계약 형식으로 기록합니다. "
    "이 tool은 최종 시간을 자동으로 선택하지 않습니다. "
    "agent가 find_common_available_slots 결과의 candidate_slots 중 하나를 직접 골라 "
    "selected_index(0-based) 또는 selected_slot과 final_slot을 채워 넘겨야 합니다. "
    "final_slot 형식은 'YYYY-MM-DD HH:MM-HH:MM'입니다. "
    "아직 최종 시간을 고르지 않았다면 final_slot은 null, needs_agent_selection은 true로 설정합니다. "
    "최종 시간을 확정했다면 needs_agent_selection은 false로 설정합니다. "
    "근거 trace를 위해 candidate_slots, busy_rows, member_names, date_from, date_to도 함께 넘겨야 합니다. "
    "reason에는 최종 선택 또는 보류 이유를 사용자가 이해할 수 있는 말로 적습니다."
)
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        ...
        try:
            ...
            busy_rows = parsed.get("rows", []) if isinstance(parsed, dict) else []
        except Exception:
            busy_rows = []

    all_members = list({*normalized_names, "나"})
...
def find_common_available_slots(
    ...
    try:
        ...
        return json.dumps({"ok": False, "tool_name": "find_common_available_slots", "error": str(e)}, ensure_ascii=False)
...
def decide_final_slot(
    ...
    try:
        ...
        return json.dumps({"ok": False, "tool_name": "decide_final_slot", "error": str(e)}, ensure_ascii=False)
...
def nana_agent(query: str) -> str:
    ...
    try:
        ...
        return json.dumps({"ok": False, "tool_name": "nana_agent", "error": str(e)}, ensure_ascii=False)
...
def kana_agent(query: str) -> str:
    ...
    try:
        ...
        return json.dumps({"ok": False, "tool_name": "kana_agent", "error": str(e)}, ensure_ascii=False)
