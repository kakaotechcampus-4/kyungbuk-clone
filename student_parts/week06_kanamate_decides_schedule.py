...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "당신은 supervisor입니다. 이미 정해진 내용을 내 저장소에 조회/생성/수정/삭제하는 개인 업무, "
            "todo·reminder 저장, 개인 참고자료(메모)나 지금까지의 앱 대화 검색은 nana_agent에게 위임하세요. "
            "외부 인물 이름이 언급됐는지와 무관하게, 외부 멤버의 대화·일정을 조회해야 하거나 여러 사람의 "
            "공통 가능 시간을 새로 조율해야 하는 업무는 kana_agent에게 위임하세요 — \"우리 팀원들 가능한 "
            "시간 찾아줘\"처럼 이름이 없어도 외부 조회가 필요하면 kana_agent입니다. 멤버 이름이 부족하면 "
            "kana_agent가 직접 사용자에게 되물을 수 있으니 supervisor가 대신 nana_agent로 돌리지 마세요. "
            "요청이 애매하면 \"이미 정해진 걸 내 저장소에 기록/조회하는가\" 아니면 \"외부 멤버 정보를 "
            "조회해야 하는가\"를 기준으로 판단하세요. "
    ...
    return [
        *week04_prompt_parts(),
        (
            ...
            "\"팀 회의\", \"우리 팀\"처럼 이미 정해진 일정을 그대로 기록·조회·삭제하는 요청은 외부 "
            "인물 이름이 없어도 개인 업무이니 그대로 처리하세요 — extract_schedule_request가 kind를 "
            "group_schedule로, members를 [\"팀\"]처럼 실제 사람 이름이 아닌 값으로 분류해도 그 결과만 "
            "보고 거절하면 안 됩니다. "
            "반면 여러 사람의 공통 가능 시간을 새로 찾거나 외부 멤버의 일정·발화를 조회해야 하는 요청은 "
            "이름이 있든 없든(\"우리 팀원들 가능한 시간 찾아줘\" 포함) 당신이 접근할 수 없는 정보가 필요한 "
            "일이니 직접 처리하지 말고, 근거 없는 답을 지어내지 말고 담당이 아니라고 한두 문장으로 짧게 알리세요. "
            "요청에 개인 업무와 외부 정보가 섞여 있으면, 개인 업무는 처리하고 외부 정보가 필요한 "
            "부분만 위와 같이 짧게 알리세요."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            ...
            "\"우리 팀원들\"처럼 구체적인 멤버 이름 없이 위임된 요청은 이름을 추측해서 tool을 호출하지 "
            "말고, 어떤 멤버인지 먼저 사용자에게 되물으세요. "
            ...
            "여기 없는 일정을 만들어내지 마세요. "
            "busy_rows를 확인한 뒤에는 그 busy_rows와 겹치지 않는 시간대를 직접 골라 candidate_slots로 "
            "find_common_available_slots를 호출해 후보를 검증하고, 이어서 그중 하나를 selected_index와 "
            "final_slot으로 골라 decide_final_slot을 호출해 최종 확정하거나, 아직 고를 수 없으면 "
            "needs_agent_selection=true로 보류 상태를 남기세요. 시간을 스스로 계산해서 tool 없이 확정하지 마세요. "
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "이 tool은 공통 가능 시간을 자동으로 계산하지 않습니다. 앞선 조회 결과(collect_member_schedules 등)의 "
    "busy_rows를 그대로 복사해 인자로 넘기고, 그 busy_rows를 당신이 직접 읽어 어떤 항목과도 겹치지 않는 "
    "시간대를 candidate_slots로 골라 넘겨야 합니다. candidate_slots의 각 항목은 date(YYYY-MM-DD), "
    "start_time(HH:MM), end_time(HH:MM), duration_minutes, reason을 모두 포함해야 합니다. "
    "이 tool 호출 결과만으로 사용자에게 답을 끝내지 말고, 검증된 candidate_slots를 근거로 이어서 "
    "decide_final_slot을 호출해 최종 시간을 확정하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool은 최종 회의 시간을 자동으로 선택하지 않습니다. find_common_available_slots가 검증한 "
    "candidate_slots 중에서 당신이 직접 고른 항목을 selected_index(또는 selected_slot)와 "
    "'YYYY-MM-DD HH:MM-HH:MM' 형식의 final_slot으로 넘겨야 확정됩니다. 아직 어떤 후보를 고를지 확신할 수 "
    "없거나 사용자 확인이 더 필요하면 final_slot은 비워두고(null) needs_agent_selection을 true로 넘겨 "
    "보류 상태임을 남기세요. 후보를 확정할 때는 final_slot과 함께 reason에 그 시간을 고른 이유를 짧게 "
    "남기고, candidate_slots·busy_rows·member_names·date_from·date_to도 함께 넘겨 최종 결정의 근거를 "
    "trace에 남기세요."
)
...
def kana_agent(query: str) -> str:
    ...
    for event in events:
        ...
        if not isinstance(content, dict):
            continue
        final_slot_payload = content
        if content.get("final_slot") and not content.get("needs_agent_selection"):
