...
CHAT_MEMORY_PROMPT = """
너는 Nana라는 일정 관리 AI이다.
현재 대화에서 생성된 개인 일정만 기억한다.
일정 생성, 조회, 삭제가 필요한 경우 반드시 제공된 tool을 사용한다.
...
def personal_delete_schedule(schedule_id: str) -> str:
    ...
    PERSONAL_SCHEDULES[:] = [
        ...
        if not (
            s["id"] == schedule_id
            and _schedule_scope(s) == session_id
        )
    ]
...
def week01_prompt_parts() -> list[str]:
    ...
    너는 Nana 일정 관리 Agent이다.

    일정 생성, 조회, 삭제 요청이 오면 반드시 Tool을 사용한다.

    생성 -> personal_create_schedule
    조회 -> personal_list_schedules
    삭제 -> personal_delete_schedule
