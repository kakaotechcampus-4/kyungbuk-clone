...
# 현재 채팅 기억 관련 공통 system prompt.
# Week 1 개인 일정은 DB가 아니라 이 대화 안에서만 유지되는 임시 메모리라는 점을 모델에게 알려준다.
CHAT_MEMORY_PROMPT = (
    "개인 일정은 지금 이 대화 안에서만 기억하는 임시 메모리에 저장된다. "
    "다른 대화에서 만든 일정은 보이지 않으니, 현재 대화에서 만든 일정만 대상으로 다룬다."
)
...
def personal_create_schedule(
    ...
    # 모델이 만든 인자들을 하나의 일정 dict로 묶는다.
    # attendees가 None이면 빈 list로 바꾸고, 현재 대화 범위를 session_id에 함께 기록한다.
    schedule = {
        "id": _new_personal_id(),
        "title": title,
        "date": date,
    ...
    # Week 1 tool은 SQLite/App store를 호출하지 않고 임시 일정 dict만 그대로 돌려준다.
    return _json(
        {
            "ok": True,
...
def personal_list_schedules(date_from: str | None = None, date_to: str | None = None) -> str:
    ...
    # _current_session_schedules()는 PERSONAL_SCHEDULES를 수정하지 않고 현재 대화 범위만 새 list로 골라준다.
    schedules = _current_session_schedules()
    # 날짜 비교는 YYYY-MM-DD 문자열 기준으로 충분하다.
...
def personal_delete_schedule(schedule_id: str) -> str:
    ...
    # 리스트 객체 자체는 유지하면서 내용만 교체한다(PERSONAL_SCHEDULES[:] 대입).
    # 같은 schedule_id라도 다른 대화 범위의 일정은 남겨 둔다.
    PERSONAL_SCHEDULES[:] = [
        schedule
        for schedule in PERSONAL_SCHEDULES
        if not (schedule.get("id") == schedule_id and _schedule_scope(schedule) == session_id)
    ]
    # 삭제 전후 길이를 비교해 실제로 지워졌는지 판단한다.
...
def week01_prompt_parts() -> list[str]:
    ...
    nana_prompt = (
        ...
        "개인 일정 생성, 조회, 삭제가 필요하면 반드시 알맞은 tool을 호출한 뒤 그 결과를 바탕으로 짧게 답한다. "
        "삭제 요청은 사용자가 말한 schedule_id를 personal_delete_schedule에 그대로 전달한다."
    )
    return [CHAT_MEMORY_PROMPT, nana_prompt]
