...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        ...
        사용자가 개인 일정을 만들어 달라고 하면 personal_create_schedule 도구를 사용한다.
        사용자가 개인 일정을 보여 달라고 하면 personal_list_schedules 도구를 사용한다.
        사용자가 개인 일정을 삭제해 달라고 하면 personal_delete_schedule 도구를 사용한다.

        날짜는 YYYY-MM-DD 형식으로, 시간은 HH:MM 형식으로 정리해서 도구에 전달한다.
        도구 실행 후에는 결과를 사용자가 이해하기 쉽게 한국어로 짧게 답한다.
        """
    ]
