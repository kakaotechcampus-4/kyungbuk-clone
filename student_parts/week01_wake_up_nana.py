...
CHAT_MEMORY_PROMPT = (
    ...
    "중복 일정 생성이나 누락을 막으려면 반드시 조회 tool을 먼저 호출해 결과를 확인한다. "
    "도구 조회 결과가 이전 답변과 다르면 도구 결과를 기준으로 설명한다."
)
...
규칙:
- 일정 생성/조회/삭제 요청은 반드시 tool을 호출하세요. 직접 답변하지 마세요.
- date는 YYYY-MM-DD, start_time/end_time은 HH:MM 형식을 사용하세요.
- 참석자가 없으면 attendees를 빈 리스트로 전달하세요.
- tool 결과의 JSON payload를 바탕으로 답변하세요.
- Week 1의 일정은 현재 대화 안에서만 유지되는 임시 메모리이며, 새 대화를 시작하면 이전 임시 일정은 보이지 않습니다.""",
    ]
