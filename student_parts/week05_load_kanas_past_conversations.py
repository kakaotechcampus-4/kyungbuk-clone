...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    temp_schedules = [
        ...
        if _schedule_scope(schedule) == session_id and schedule.get("id") not in sqlite_ids
    ]
...
def _collect_member_schedules(
    *,
    ...
    for schedule in personal_schedules:
        ...
        date = request.date or schedule.get("date")
        if normalized_date_from and (not date or date < normalized_date_from): continue
        if normalized_date_to and (not date or date > normalized_date_to): continue
        my_rows.append(
            {
                "member_name": PERSONAL_SHARED_MEMBER_NAME,
                "title":request.title or schedule.get("title"),
                "date": date,
                "start_time": request.start_time or schedule.get("start_time"),
                "end_time": request.end_time or schedule.get("end_time"),
