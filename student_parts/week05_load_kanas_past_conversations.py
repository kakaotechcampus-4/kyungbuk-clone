...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    persisted_ids = {row.get("schedule_id") for row in persisted}

    current_scope = current_session_scope() or DEFAULT_SESSION_SCOPE
    temp_schedules = [
        schedule
        for schedule in PERSONAL_SCHEDULES
        if _schedule_scope(schedule) == current_scope and schedule.get("id") not in persisted_ids
    ]

    return [*persisted, *temp_schedules]
