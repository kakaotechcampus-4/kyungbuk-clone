...
def _collect_member_schedules(
    *,
    ...
    args = {"member_names": normalized_members, "date_from": normalized_from, "date_to": normalized_to}

    external_raw = call_mcp_tool_sync("extract_schedules_from_history", args)
    external_payload = json.loads(external_raw)
    external_rows = external_payload.get("rows", external_payload) if isinstance(external_payload, dict) else external_payload
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "## Week 5 외부 대화/일정 조율 tool 선택 기준\n"
        "- 외부 팀원과 예전에 나눈 대화를 찾아야 할 때는 search_previous_conversations로 먼저 검색하고,\n"
        "  특정 대화 전체 내용을 봐야 하면 load_conversation_messages로 conversation_id의 메시지를 불러옵니다.\n"
        "- 특정 멤버들의 바쁜 시간을 이전 대화 기록에서 추출해야 할 때는 extract_schedules_from_history를 사용합니다.\n"
        "- 외부 공유 일정 저장소에 등록된 일정 자체를 확인하고 싶을 때는 list_shared_schedules를 사용합니다.\n"
        "- 나와 다른 멤버들의 일정을 한 번에 모아 회의 가능 시간을 검토해야 할 때는 collect_member_schedules를 사용합니다.\n"
        "- 여러 사람의 최종 공통 가능 시간을 정하는 것은 Week 6 범위입니다.\n",
    ]
