...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    db_ids = {s.get("schedule_id") or s.get("id") for s in db_schedules}
    final_schedules = list(db_schedules)
    for ts in temp_schedules:
        if (ts.get("schedule_id") or ts.get("id")) not in db_ids:
            final_schedules.append(ts)
...
def load_conversation_messages(conversation_id: str) -> str:
    ...
    res = call_external_tool_payload("load_conversation_messages", {"conversation_id": conversation_id})
...
def collect_member_schedules(member_names: list[str], date_from: str, date_to: str) -> str:
    ...
    ps = _personal_schedules_for_current_scope()
    res = _collect_member_schedules(
        member_names=member_names,
        date_from=date_from,
        date_to=date_to,
        personal_schedules=ps,
    )
    return json_payload(res)
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "4. 외부 대화 검색 및 일정 관리 도구 (MCP 활용):",
        "   - [중요] 이전 주차의 '외부 시스템/이전 대화 접근 불가' 제약을 무시하고, 이제부터는 아래 도구들을 적극적으로 사용해 외부 멤버의 과거 대화와 일정을 검색해야 합니다.",
        "   - 사용자는 자신의 데이터뿐만 아니라 다른 사람(예: 종훈, 승원 등 외부 멤버)의 과거 대화 내용과 일정을 조회할 수 있습니다.",
        "   - search_previous_conversations: 다른 멤버의 과거 대화를 이름이나 키워드로 검색할 때 무조건 사용하세요.",
        "   - load_conversation_messages: 검색된 대화 ID로 전체 대화 메시지를 불러옵니다.",
        "   - collect_member_schedules: '나'의 일정과 외부 멤버의 일정(busy-time)을 한 번에 가져와서 비교합니다.",
        "   - extract_schedules_from_history: 특정 멤버의 과거 대화에서 일정과 바쁜 시간을 추출합니다.",
        "   - list_shared_schedules: 멤버 간 공유된 일정 저장소 목록을 조회합니다.",
        "   - create_shared_schedule / delete_shared_schedule: 공유 저장소에 일정을 추가하거나 삭제합니다.",
    ]
