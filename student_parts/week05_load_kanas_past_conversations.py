...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    saved_ids = {s.get("schedule_id") for s in saved_schedules}
    unique_temporary = [s for s in temporary_schedules if s.get("id") not in saved_ids]

    return saved_schedules + unique_temporary
    ...
    my_rows = [
        {
            ...
            "date": s.get("date") or "날짜 미정",
            "start_time": s.get("start_time") or "미정",
            "end_time": s.get("end_time") or "미정",
            "notes": _my_schedule_notes(_structured_request_from_schedule_row(s)),
        }
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        (
            "Week 5부터는 팀원들의 이전 대화 기록과 바쁜 시간을 조회할 수 있습니다. "
            "search_previous_conversations로 검색한 대화를 load_conversation_messages로 읽고, "
            "extract_schedules_from_history나 collect_member_schedules로 바쁜 시간을 모읍니다. "
            "외부 공유 일정 저장소는 list_shared_schedules로 조회하고, 필요하면 "
            "create_shared_schedule/delete_shared_schedule로 등록/삭제합니다."
        ),
    ]
