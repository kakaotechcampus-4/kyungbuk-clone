...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    # SQLite 에 저장된 내 일정을 조회(읽기)
    store = AppSQLiteStore(CONFIG.app_db_path)
    saved = store.list_schedules(limit=50)

    # 각 row 를 돌면서 schedule_id 나 id 값이 존재하는 경우
    # 해당 값을 saved_ids 에 저장
    saved_ids = {
        str(row.get("schedule_id") or row.get("id"))
        for row in saved
        if row.get("schedule_id") or row.get("id")
    }

    # 현재 대화 범위(scope) 를 scope 에 저장
    scope = current_session_scope()
    # 현재 대화 범위이면서 DB 에 저장되지 않고 PERSONAL_SCHEDULES 에만 있는(메모리)
    # 일정들을 pending 에 저장
    pending = [
        schedule
...
def _collect_member_schedules(
    *,
    ...
    for row in personal_schedules:
        # Structured_Request 로 변환(구조와 값(날짜,시간)을 표준화 하기 위해) 후
        # my_rows 에 내 일정들을 저장
    ...
    # 외부 멤버 rows: MCP tool 조회 결과(JSON 문자열)를 dict 로 파싱한 후 rows 만 꺼내
    # external_payload 에 저장
...
def load_conversation_messages(conversation_id: str) -> str:
    ...
    # MCP tool 을 호출해 결과(dict)를 payload 에 저장
    payload = call_external_tool_payload(
        "load_conversation_messages", # 호출하는 tool name
        {"conversation_id": conversation_id}, # 전달하는 매개변수(args(dict))
    )

    # dict 형태인 payload 를 JSON 형태로 변환 후 return
...
def list_shared_schedules(
    ...
    # tool 에서 받은 인자들을 dict 형태로 변환 후 args 에 저장
    args = {
        "member_names": member_names,
        "date_from": date_from,
        "date_to": date_to,
    ...
    # 공유 일정 저장소 rows 조회 결과를 JSON 형태로 그대로 반환
    # 필터가 없을 시 기본 공유 일정은  
    # fixed/external_people_store.py의 list_shared_schedules 가 로직 수행
...
def collect_member_schedules(member_names: list[str], date_from: str, date_to: str) -> str:
    ...
    # 나의 일정과 외부 멤버의 일정을 helper 함수를 호출해 dict 형태로 묶어 payload 에 저장
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "'공유 일정에 등록해줘'처럼 공유 저장소가 목적지로 언급되면 "
        "personal_create_schedule이 아니라 반드시 create_shared_schedule로 등록하고, 삭제는 delete_shared_schedule을 써. "
