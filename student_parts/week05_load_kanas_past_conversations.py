...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    ids = [schedules.get('schedule_id') for schedules in sqlite_schedules]
    exclusive_schedules = [schedule for schedule in personal_schedules if schedule.get('id') not in ids]

    ret = sqlite_schedules
    ret.extend(exclusive_schedules)
...
class SturcturedSchedule(BaseModel):
    member_name: str = ""
    title: str = ""
    date: str
    start_time: str
...
def _collect_member_schedules(
    *,
    ...
    normalized_date_from, normalized_date_to = normalize_external_schedule_date_bounds(member_names=member_names, date_from=date_from, date_to=date_to)
    ...
    res = call_mcp_tool_sync("extract_schedules_from_history", args) # list[member_name, title, date, start_time, end_time, notes, source_conversation_id]
...
def search_previous_conversations(
    ...
    ret = call_mcp_tool_sync("search_previous_conversations", args)
...
def load_conversation_messages(conversation_id: str) -> str:
    ...
    ret = call_external_tool_payload("load_conversation_messages", args)
...
def extract_schedules_from_history(member_names: list[str], date_from: str, date_to: str) -> str:
    ...
    ret = call_mcp_tool_sync("extract_schedules_from_history", args)
...
def create_shared_schedule(
    ...
    ret = call_mcp_tool_sync("create_shared_schedule", args)
...
def delete_shared_schedule(
    ...
    ret = call_mcp_tool_sync("delete_shared_schedule", args)
...
def list_shared_schedules(
    ...
    ret = call_mcp_tool_sync("list_shared_schedules", args)
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        """
        이 프롬프트는 Week 5의 프롬프트이다. 이전 프롬프트보다 높은 우선 순위를 가진다.
        외부 멤버의 이전 대화, 외부 멤버 일정, 공유 일정 저장소를 묻는 경우에는 Week 5 MCP wrapper tool을 사용한다.

        search_previous_conversations: 외부 멤버가 과거 대화에서 말한 내용, 일정 단서, 이전 대화 제목이나 메시지를 검색해야 하는 경우
        load_conversation_messages: search_previous_conversations 결과의 conversation_id를 바탕으로 특정 외부 대화의 전체 메시지 흐름을 확인해야 하는 경우
        extract_schedules_from_history: 외부 멤버들의 이전 대화에서 특정 날짜 범위의 일정이나 바쁜 시간을 직접 조회해야 하는 경우
        collect_member_schedules: 내 일정과 외부 멤버 일정을 함께 모아 비교해야 하는 경우. 여러 사람의 바쁜 시간 확인, 회의 조율 전 busy-time 수집에는 이 도구를 우선 사용한다.
        list_shared_schedules: 외부 공유 일정 저장소에 등록된 row를 확인해야 하는 경우. "나" 일정 복사본이나 공유 저장소 상태를 확인할 때 사용한다.
        create_shared_schedule: 사용자가 공유 일정 저장소에 일정을 등록하거나 갱신하라고 명확히 요청한 경우
        delete_shared_schedule: 사용자가 공유 일정 저장소의 일정을 삭제하라고 명확히 요청한 경우
        - 외부 멤버의 과거 대화 내용을 묻는 경우에는 search_previous_conversations로 먼저 검색하고, 필요한 경우 load_conversation_messages로 전체 메시지를 불러온다.
        - 내 일정과 외부 멤버 일정을 함께 봐야 하는 경우에는 extract_schedules_from_history만 사용하지 말고 collect_member_schedules를 사용한다.
        - 단순 조회나 가능 시간 확인 요청에서는 create_shared_schedule과 delete_shared_schedule을 임의로 호출하지 않는다.
        - tool이 반환한 rows, schedules, messages, schedule_summary를 답변의 근거로 사용한다.
        - 일정처럼 날짜와 시간이 중요한 답변은 date, start_time, end_time, member_name, title을 확인해 구체적으로 답한다.
        - search_previous_conversations의 query에는 빈 문자열이 포함되어서는 안된다.
        """,
    ]
